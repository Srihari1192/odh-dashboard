...
describe('Feature Store Features Page Validation', () => {
  ...
  const testFeatureTags = 'enrichment=location';
    ...
    cy.fixture('e2e/featureStoreResources/testFeatureStoreResources.yaml', 'utf8')
      ...
      .then(() => {
        ...
        cy.get<string>('@routeUrl').then((routeUrl) => {
          getFeatureCount(routeUrl, testData.feastCreditScoringProject);
        });
      });
  });
  ...
  it(
    ...
    () => {
      ...
      cy.get<number>('@featureCount').then((count) => {
        featureStoreGlobal.shouldHaveTotalCount(count);
      });
      ...
      featureDetails
        ...
        .shouldHaveFeatureTagsText(testFeatureTags);
      cy.get(`pre#${testFeatureName}`).should('be.visible');
