...
} from '#~/__tests__/cypress/cypress/utils/e2eUsers.ts';
...
const users = [
  { name: 'Cluster Admin', creds: HTPASSWD_CLUSTER_ADMIN_USER },
  { name: 'Test User', creds: LDAP_CONTRIBUTOR_USER },
];
...
describe('Feature Store UI default content validation', () => {
  ...
  Cypress._.each(users, (user) => {
    it(
      `Verify Feature Store UI default contents across all pages with ${user.name} login`,
      { tags: ['@Dashboard', '@FeatureStore', '@FeatureFlagged'] },
      () => {
        // Overview Page
        cy.step('Verify Overview Page');
        featureStoreGlobal.visitFeatureStore(testData.feastCreditScoringProject, user.creds);
        cy.contains('Description of feature store').should('be.visible');
        ...
        cy.contains(
          'Select a feature store workspace to view and manage its feature views.',
        ).should('be.visible');
